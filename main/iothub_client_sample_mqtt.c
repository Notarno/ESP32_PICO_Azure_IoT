...
#define IOTHUB_CONNECTION_STRING "HostName=C-Test-Hub.azure-devices.net;DeviceId=PICO-1;SharedAccessKey=FJ2fwO5328q7wnn4QFEOaWSb8NG++xh1jaRwJB7QkZk="

#define MESSAGE_COUNT CONFIG_MESSAGE_COUNT
#define DOWORK_LOOP_NUM     3

#define UART1_TXD (GPIO_NUM_10) 
#define UART1_RXD (GPIO_NUM_9)
#define UART1_RTS (UART_PIN_NO_CHANGE)
#define UART1_CTS (UART_PIN_NO_CHANGE)

#define BUF_SIZE (1024)

static const char* connectionString = IOTHUB_CONNECTION_STRING;

static int callbackCounter;
static char msgText[1024];
static bool g_continueRunning;

/* Configure a buffer for incoming data */
uint8_t * iotMsg;
...
static void UARTTask(){	
	iotMsg  = (uint8_t *) malloc(BUF_SIZE);
	int len = 0;
	
	while(len == 0){
		/* Prevents the Watch Dog Timer from Triggering */
		TIMERG0.wdt_wprotect = TIMG_WDT_WKEY_VALUE;
		TIMERG0.wdt_feed = 1;
		TIMERG0.wdt_wprotect = 0;
		
		len = uart_read_bytes(UART_NUM_1, iotMsg, 10, 20/portTICK_RATE_MS);
...
static void UARTInit(){
	/* Configure parameters of Uart1 driver, communication pins and install the driver */
...
// built in certificate stores.
            ...
            if (IoTHubClient_LL_SetMessageCallback(iotHubClientHandle, ReceiveMessageCallback, &receiveContext) != IOTHUB_CLIENT_OK){
                ...
                do{
                    ...
                    if ((MESSAGE_COUNT == 0 || iterator < MESSAGE_COUNT) && iterator <= callbackCounter && (difftime(current_time, sent_time) > ((CONFIG_MESSAGE_INTERVAL_TIME) / 1000))){
                        sprintf_s(msgText, sizeof(msgText), "{Device-Id: \"ESP32 PICO\", message: %s}", iotMsg);
